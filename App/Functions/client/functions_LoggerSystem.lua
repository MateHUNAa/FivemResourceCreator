...
Functions = {
     ...
     unloadModel = (function(model)
          Logger:Debug("^2Removing Model^7: '^6" .. model .. "^7'")
          SetModelAsNoLongerNeeded(model)
     end),
     ---@param dict string
     loadAnimDict = (function(dict)
          if not HasAnimDictLoaded(dict) then
               Logger:Debug("^2Loading Anim Dictionary^7: '^6" .. dict .. "^7'")
     ...
     unloadAnimDict = (function(dict)
          ...
          Logger:Debug("^2Removing Anim Dictionary^7: '^6" .. dict .. "^7'")
          RemoveAnimDict(dict)
     end),
     ---@param dict string
     loadPtfxDict = (function(dict)
          if not HasNamedPtfxAssetLoaded(dict) then
               Logger:Debug("^2Loading Ptfx Dictionary^7: '^6" .. dict .. "^7'")
     ...
     unloadPtfxDict = (function(dict)
          Logger:Debug("^2Removing Ptfx Dictionary^7: '^6" .. dict .. "^7'")
     ...
     makeProp = (function(data, freeze, synced)
          ...
          if isDebug then
               ...
               Logger:Debug("^1Prop ^2Created^7: '^6" ..
                    prop ..
                    "^7' | ^2Hash^7: ^7'^6" ..
                    data.prop ..
     ...
     makeBlip = (function(data)
          ...
          -- Logger:Debug("^6Blip ^2created for location^7: '^6" .. data.name .. "^7'")
          return blip
     end),
     ...
     makePed = (function(model, data, options)
          if not IsModelValid(model) then
               return Logger:Debug("^4Invalid Model^7: '^6" .. model .. "^7'")
          end
          ...
          Logger:Debug(("^6Ped  ^2created for location^7: '^6%s^7'"):format(model))
     end),
     ---@param entity number EntityId
     destoryProp = (function(entity)
          Logger:Debug("^2Destroying Prop^7: '^6" .. entity .. "^7'")
     ...
     loadDrillSound = (function()
          Logger:Debug(("^2Loading Drill Sound Banks^7"))
     ...
     unloadDrillSound = (function()
          Logger:Debug(("^2Removeing Drill Sounds Banks^7"))
     ...
     lookAtMe = (function(entity)
          if DoesEntityExist(entity) then
               if not IsPedHeadingTowardsPosition(entity, GetEntityCoords(PlayerPedId()), 30.0) then
                    ...
                    Logger:Debug("^2Turning Player to^7: '^6" .. entity .. "^7'")
                    Wait(1500)
               end
          end
     end),
     ...
     lookEnt = (function(entity)
          if type(entity) == "vec3" then
               if not IsPedHeadingTowardsPosition(PlayerPedId(), entity, 10.0) then
                    ...
                    Logger:Debug("^2Turning Player to^7: '^6" .. json.encode(entity) .. "^7'")
                    Wait(1500)
               end
          else
               if DoesEntityExist(entity) then
     ...
     toggleItem = (function(give, item, amount)
          if (GetResourceState("mCore") == "started") then
            lib.callback.await("mCore:toggleItem", false, give,item,amount)
          else
            Logger:Error("mCore is not running Toggle Item is not initialized in this context !")
          end
     end),
     ---@param items string
     ---@param amount number
     HasItem = (function(items, amount)
          ...
          if count >= amount then
               Logger:Debug(("^3HasItem^7: ^5FOUND^7 ^3     %s ^7/^3 %s %s^7"):format(count, amount, items))
               return true
          else
               Logger:Debug(("^3HasItem^7: ^1NOT FOUND ^2 %s ^7"):format(tostring(items)))
               return false
          end
     end),
